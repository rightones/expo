...
open class FileDownloader(context: Context, private val client: OkHttpClient) {
  ...
  companion object {
    ...
    fun getExtraHeadersForRemoteUpdateRequest(
      ...
      ManifestMetadata.getExtraClientParams(database, configuration)?.let {
        extraHeaders.put("Expo-Extra-Client-Params", Dictionary.valueOf(it.mapValues { elem -> StringItem.valueOf(elem.value) }))
      }
