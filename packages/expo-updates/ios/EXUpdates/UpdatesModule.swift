  ...
  public func definition() -> ModuleDefinition {
    ...
    AsyncFunction("getExtraClientParamsAsync") { (promise: Promise) in
      ...
      updatesService.database.databaseQueue.async {
        do {
          promise.resolve(try updatesService.database.extraClientParams(withScopeKey: scopeKey))
        } catch {
    ...
    AsyncFunction("setExtraClientParamsAsync") { (extraClientParams: [String: String], promise: Promise) in
      guard let updatesService = updatesService,
        let config = updatesService.config,
        config.isEnabled else {
        throw UpdatesDisabledException()
      ...
      updatesService.database.databaseQueue.async {
        do {
          try updatesService.database.setExtraClientParams(extraClientParams, withScopeKey: scopeKey)
          promise.resolve(nil)
        } catch {
          promise.reject(error)
        }
